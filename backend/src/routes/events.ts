...
import { type Event, type RSVP } from "../types.ts";
...
const getEvent = <Type>(eventId: Type) => {
...
router.get("/", (_req, res) => {
  ...
  res.json(events.map((event: Event) => joinRSVPs(joinHost(event) as Event)));
});
...
router.post("/:id/rsvp", (req, res) => {
  ...
  let [rsvp]: RSVP[] | unknown[] = getRSVP.all({ eventId, email });
  if (rsvp) {
