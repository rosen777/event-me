...
export const getUser = <Type>(userId: Type) => {
...
router.get("/", (_req, res) => {
  ...
  const users = listUsers.all() as User[];
  res.json(users);
});
...
router.get("/:id", (req, res) => {
  ...
  const user: User | unknown = getUser(id);
  if (!user) {
...
router.patch("/:id", (req, res) => {
  ...
  const updated: User = getUser(userId) as User;
  res.json(updated);
});
...
router.delete("/:id", (req, res) => {
  ...
  const user: User = getUser(userId) as User;
  if (!user) {
    res.status(404).json({ error: "User not found" });
  }
